...
def run(screen, player, player_skin):
    ...
    listposition = random.randint(
        0, 2)  # wählt eine Position für die Liste Colors aus
    pygame.Rect(player[0], player[1], 44, 44)  # Spieler
    field = pygame.Rect(player[0], player[1], 44,
                        44)  # das Feld, wo sich der Spieler gerade befindet
    pygame.draw.rect(
        screen, colors[listposition], field
    )  # Zeichnet ein Rechteck auf der Position auf dem sich der Spieler befindet
...
def check_counter(screen, remo_list, coinskin, coins_rect):
    for remove in remo_list:
        if (remove.colliderect(coins_rect[0])
                or remove.colliderect(coins_rect[1])
                or remove.colliderect(coins_rect[2])
                or remove.colliderect(coins_rect[3])
                or remove.colliderect(coins_rect[4])
                or remove.colliderect(coins_rect[5])
                or remove.colliderect(coins_rect[6])
                or remove.colliderect(coins_rect[7])
                or remove.colliderect(coins_rect[8])
                or remove.colliderect(coins_rect[9])):
            screen.blit(coinskin,
                        (remove[0],
                         remove[1]))  # Es erscheint eine Münze auf dem Screen
...
def move(screen, player_coords, statement):
    ...
    if (
            statement == False
    ):  # Dieser Block erstellt eine Liste mit den Koordinaten von jedem Block
        list_coords = []
        while x != 34 and y != 21:
            buttons = pygame.Rect(0 + ((50 - 1) * x + 7),
                                  0 + ((50 - 1) * y + 7), 44, 44)
...
def collideplayer(player, list_coords, remo_list, statement):
    ...
    if (statement == True
        ):  # Dieser Block schaut ob der Spieler das Feld schon besucht hat
        ...
        return remo_list
            ...
            statement == False
    ):  # Dieser Block fügt die Koordinaten vom Spieler in die Remove Liste
        for blocks in list_coords:
            if player_rect.colliderect(blocks):
                x = blocks
        if x in list_coords:
    ...
    if (
            statement == 5
    ):  # Dieser Block blendet den Game Over Screen ein bei Collision Detection
        s = None
        for blocks in list_coords:
            if player_rect.colliderect(blocks):
                x = blocks
