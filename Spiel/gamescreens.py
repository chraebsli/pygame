...
def titlescreen(data, data_1):
    ...
    screen.blit(play_button,
                (buttons_titlescreen_xy[0], buttons_titlescreen_xy[1]))
    screen.blit(skin_button,
                (buttons_titlescreen_xy[0], buttons_titlescreen_xy[1]))
    screen.blit(quit_button,
                (buttons_titlescreen_xy[0], buttons_titlescreen_xy[1]))
    screen.blit(howto_button,
                (buttons_titlescreen_xy[0], buttons_titlescreen_xy[1]))
    screen.blit(leaderboard_button,
                (buttons_titlescreen_xy[0], buttons_titlescreen_xy[1]))
    ...
    sound_surface = second_font.render(f"SOUNDS: {sounds.upper()}", False,
                                       (255, 255, 255))
    ...
    for event in pygame.event.get():
        ...
        # detect mousepos on categories
            ...
            x, y = event.pos
            if (x > play_button_rect[0] and y > play_button_rect[1]
                    and x < play_button_rect[2] and y < play_button_rect[3]):
                screenmode = "timer"  # screenmode='gamescreen' # to skip seq
                click.play()
                send_data = True
            elif (x > skin_button_rect[0] and y > skin_button_rect[1]
                  and x < skin_button_rect[2] and y < skin_button_rect[3]):
                screenmode = "skinscreen"
                click.play()
                send_data = True
            elif (x > quit_button_rect[0] and y > quit_button_rect[1]
                  and x < quit_button_rect[2] and y < quit_button_rect[3]):
                screenmode = "quitscreen"
                click.play()
                send_data = True
            elif (x > howto_button_rect[0] and y > howto_button_rect[1]
                  and x < howto_button_rect[2] and y < howto_button_rect[3]):
                screenmode = "howto"
                click.play()
                send_data = True
            elif (x > leaderboard_button_rect[0]
                  and y > leaderboard_button_rect[1]
                  and x < leaderboard_button_rect[2]
                  and y < leaderboard_button_rect[3]):
...
def gamescreen(data, data_2, remo_list, random_number):
    ...
    x_color = random.randint(
        0, 2
    )  # Falls der Spieler ohne die Wände zu konfigurien, das Spiel startet
    x_color_one = random.randint(
        0,
        2)  # Falls der Spieler ohne den Pfad zu konfigurien, das Spiel startet
    # Sounds
    ...
    try:
        collision_detct.drawing(screen, walls_rect, final_index_w,
                                random_number, color, False)
    except NameError:
        collision_detct.drawing(screen, walls_rect, 1, random_number, red,
                                True)
    ...
    try:
        collision_detct.playerpath(remo_list, screen, player_xy, color_one,
                                   final_index_p, False)
    except NameError:
    ...
    if keys[0] or keys[1] or keys[2] or keys[3]:
        ...
        remo_list = collision_detct.collideplayer(player_xy, list_coords,
                                                  remo_list, False)
    ...
    if keys[4]:
        ...
        screenmode = collision_detct.collideplayer(player_xy, list_coords,
                                                   remo_list, 5)
    ...
    if player_xy == end_xy:
        ...
        print(
            f"You ended this round as {playername} with {points} points in {played_time} minutes"
        )
        if points >= 150:
    ...
    if send_data is True:
        ...
        return screenmode
    gamefunctions.end_timer(
        t1, " to load frame")  # print how long it takes to load a frame
    ...
    while not done:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if x > 600 and y > 600 and x < 1005 and y < 735 and len(
                        playername) > 0:
                    click.play()
                    screenmode = "titlescreen"
                    send_data = True
        ...
        text_surface = base_font.render(f"Name: {playername}", True,
                                        (255, 255, 255))
...
def timer(data, timer, number):
    ...
    while run:
        ...
        screen.blit(font.render(text + " SECONDS", True, (255, 255, 255)),
                    (182 + 60, 600 + 150))
        screen.blit(font.render("GAME STARTS", True, (255, 255, 255)),
                    first_render_pos)
...
def settings(data, return_manuels, random_number):
    ...
    settings_change_wall = [
        "RAINBOW", "RANDOM", "RED", "BLUE", "GREEN", "CUSTOM"
    ]
        ...
        if event.type == pygame.KEYDOWN and done is False:
            ...
            if event.key == pygame.K_RETURN:
                if changing_wall is True and active_wall is True:
                    try:
                        if pygame.Color(custom_color) == pygame.Color(
                                custom_color):
                            done = True
                    except ValueError:
                        invalid = True
                ...
                if changing_path is True and active_path is True:
                    try:
                        if pygame.Color(custom_color_path) == pygame.Color(
                                custom_color_path):
                            done_two = True
                    except ValueError:
                        invalid = True
    ...
    screen.blit(font_one.render("IN-GAME SETTINGS", True, (255, 255, 255)),
                (320, 150))
    ...
    second_font = pygame.font.SysFont(None, 90)
    if settings_change_wall[index_wall] != "CUSTOM":
        wall_surface = second_font.render(
            f"COLOR OF WALLS: {settings_change_wall[index_wall]}",
            False,
            (255, 255, 255),
        )
    else:
        wall_surface = second_font.render(
            f"COLOR OF WALLS: {custom_color.upper()}", False, (255, 255, 255))
    if settings_change_path[index_path] != "CUSTOM":
        path_surface = second_font.render(
            f"COLOR OF PATH : {settings_change_path[index_path]}",
            False,
            (255, 255, 255),
        )
    else:
        path_surface = second_font.render(
            f"COLOR OF PATH : {custom_color_path.upper()}", False,
            (255, 255, 255))
